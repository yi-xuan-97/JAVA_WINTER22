...
//      }
      ...
      if(args[i+1].length()!=3){
        ...
        flight.setSrc(args[i+1]);

      String[] src_date = args[i+2].split("/");
      ...
      String[] src_time = args[i+3].split("/");
      if(Integer.parseInt(src_time[0])<0 || Integer.parseInt(src_time[0])>59){
        System.err.println("Please enter your time using format hh/mm,and check if it exist");
        System.exit(1);
      }

      if(args[i+4].length()!=3){
        System.err.println("Code of departure airport should be three letter");
        System.exit(1);
      }
      else
        flight.setSrc(args[i+4]);

      String[] src_date1 = args[i+5].split("/");
      if(Integer.parseInt(src_date1[0])<0 || Integer.parseInt(src_date1[0])>12){
        System.err.println("Please enter your date using format mm/dd/yyyy,and check if it exist");
        System.exit(1);
      }

      String[] src_time1 = args[i+6].split("/");
      if(Integer.parseInt(src_time1[0])<0 || Integer.parseInt(src_time1[0])>59){
        System.err.println("Please enter your time using format hh/mm,and check if it exist");
        System.exit(1);
      }
